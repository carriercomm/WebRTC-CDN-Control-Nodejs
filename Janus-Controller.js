...
Janus.sessions = {};// List of sessions

Janus.noop = function() {};

Janus.init = function(options) { // Initialization
	options = options || {};
...
function Janus(gatewayCallbacks) {
			...
			if (error) {
				...
				if(error['code'] != undefined && error['code'] != null) {
					if(error['code'] === 'ECONNREFUSED'){
						callbacks.error(error['code'] + ": Is the gateway down?");
					}else {
						callbacks.error("Failed: : " + error);
					}
				}
			} else {
				var json = JSON.parse(body);
	...
	function createHandle(callbacks) {
		...
		var options = {
			method: 'POST',
			uri: server + "/" + sessionId,
			headers: {"content-type": "application/json", "Cache-Control": "no-cache"},
			body: JSON.stringify(request)
		};
		var callback = function (error, response, body) {
			if (error) {
				Janus.log("Failed: " + error);
			} else {
				var json = JSON.parse(body);
				...
				var pluginHandle = {
					...
					sendTrickleE : function(callbacks) { sendTrickleCandidate(handleId, callbacks); },
	...
	function sendMessage(handleId, callbacks) {
		...
		var callback = function (error, response, body) {
			if (error) {
				...
				var json = JSON.parse(body);
				Janus.log("SendMessage response:");
				Janus.log(JSON.stringify(json));
				if(json["janus"] === "success") {
		...
		if(isAudioSendEnabled(media) || isVideoSendEnabled(media)) {
			...
			if(videoSupport === true && media != undefined && media != null) {
				if(media.video && media.video != 'screen') {
					...
					Janus.log("height:" + height + "-maxHeight:" + maxHeight + "-width:" + width + "-maxWidth:" + maxWidth);
